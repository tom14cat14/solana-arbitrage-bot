...
echo -e "${BLUE}💊 Arb Bot Health Check${NC}"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
...
# Check 5: No excessive errors
if [ -f "$LOG_FILE" ]; then
    ERROR_COUNT=$(grep -i "ERROR" "$LOG_FILE" 2>/dev/null | tail -100 | wc -l)
...
# Check 6: Wallet connectivity
if [ -f "$LOG_FILE" ]; then
    RPC_ERRORS=$(grep -i "RPC\|connection\|timeout" "$LOG_FILE" 2>/dev/null | tail -50 | wc -l)

    if [ $RPC_ERRORS -lt 5 ]; then
        echo -e "${GREEN}✅ RPC connectivity healthy${NC}"
        HEALTH_SCORE=$((HEALTH_SCORE + 1))
    else
        echo -e "${YELLOW}⚠️  RPC connectivity issues (${RPC_ERRORS} recent issues)${NC}"
