...
use crate::shredstream_price_monitor::ShredStreamPriceMonitor;
use crate::real_shredstream_client::RealShredStreamClient;
use tracing::error;
...
pub struct ArbitrageEngine {
    ...
    /// Real-time price cache for all monitored tokens across DEXs
    price_cache: HashMap<String, TokenPrice>,
    /// ShredStream price monitor for ultra-low latency price feeds
    shredstream_monitor: ShredStreamPriceMonitor,
    /// Real ShredStream gRPC client for live blockchain data
    real_shredstream_client: Option<RealShredStreamClient>,
...
impl ArbitrageEngine {
    ...
    pub async fn new(
        ...
        info!("🔧 Step 1: Creating ShredStream price monitor...");
        let shredstream_monitor = ShredStreamPriceMonitor::new(shreds_endpoint.clone());
        info!("✅ Step 1 complete: ShredStream price monitor created");
        ...
        // Initialize real ShredStream client for live trading
        let real_shredstream_client = if enable_real_trading {
            info!("🌊 Initializing Real ShredStream gRPC client for live trading");
            Some(RealShredStreamClient::new(shreds_endpoint.clone()))
        } else {
    ...
    pub async fn start_monitoring(&mut self) -> Result<()> {
        ...
        loop {
            ...
            if last_cleanup.elapsed().as_secs() >= 30 {
                ...
                self.shredstream_monitor.cleanup_old_prices();
    ...
    /// Update prices from real ShredStream gRPC client or fallback monitor
    async fn update_shredstream_prices(&mut self) -> Result<()> {
        // Use real ShredStream client if available (live trading mode)
        if let Some(real_client) = &mut self.real_shredstream_client {
            // Process a single cycle to generate new price updates
            let real_price_updates = match real_client.process_single_cycle().await {
                Ok(updates) => updates,
                Err(e) => {
                    warn!("Failed to process ShredStream cycle: {}", e);
                    Vec::new()
                }
            };

            if !real_price_updates.is_empty() {
                info!("📡 Processing {} real price updates from gRPC ShredStream", real_price_updates.len());

                for price_update in real_price_updates {
                    // Convert to our internal format and cache
                    let token_price = TokenPrice {
                        price: price_update.price_sol,
                        liquidity: price_update.liquidity,
                        last_updated: price_update.timestamp,
                        dex_name: format!("{}_{}",
                            self.get_dex_name_from_program_id(&price_update.dex_program_id),
                            &price_update.dex_program_id[0..8] // Short identifier
                        ),
        ...
        // Fallback to monitor-based approach
        match self.shredstream_monitor.receive_shredstream_data().await {
            Ok(events) => {
                for event in events {
                    if let Err(e) = self.shredstream_monitor.handle_price_event(event).await {
                        debug!("ShredStream price event error: {}", e);
                    }
                }
